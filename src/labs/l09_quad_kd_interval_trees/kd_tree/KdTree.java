package labs.l09_quad_kd_interval_trees.kd_tree;
...
public class KdTree {
    ...
    private int compare(Point2D point, Point2D other, int depth) {
        int cmp;
        if (depth % 2 == 0) {
            cmp = Double.compare(point.getX(), other.getX());
            if (cmp == 0) {
                cmp = Double.compare(point.getY(), other.getY());
            }
            return cmp;
        }
        cmp = Double.compare(point.getY(), other.getY());
